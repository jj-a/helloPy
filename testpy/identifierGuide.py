'''
<식별자(identifier) 명칭의 작성 스타일>

변수나 함수명 같은 식별자(identifier)의 명칭을 작성할 때 이름을 정의하는 것도 코딩 스타일 중 일부입니다. 대표적인 표기법은 다음 네 가지가 있습니다.

# 카멜 표기법(Camel Case) #
여러 단어를 연달아 사용할 때 각 단어의 첫 글자를 대문자로 적되, 맨 앞에 오는 글자는 소문자로 표기하는 것입니다.
낙타의 등에 있는 혹과 같다고 하여 카멜(Camel) 표기법이라고 부릅니다.
Java의 권장 표기법입니다.
...
접두어에 자료형을 붙이는 표기법입니다. 요즈음은 잘 사용하지 않는 스타일입니다.
언어의 종류가 다양한 만큼 자료형도, 문서 데이터도 다양해졌기 때문에 접두어가 의미가 없어졌기 때문 입니다.
예) strName, bBusy, szName ...

# 스네이크 표기법(Snake Case) #
단어 사이에 언더바를 넣어서 표기하는 방법입니다.
하지만 "_" 이나 "__"로 시작되는 식별자들은 많은 언어에서 확장으로 예약되어 있는 경우가 많으므로 사용하지 않는 것이 관례입니다.

대부분의 경우 카멜 기법과 파스칼 표기법을 적절하게 조합하여,
변수명이나 함수명은 카멜 표기법을 따르고 클래스명은 파스칼 표기법을 따르는 작성 스타일,
혹은 변수명은 카멜로 표기하고 함수와 클래스명은 파스칼로 표기하는 작성 스타일이 대세입니다.
전자는 Java, 후자는 C++에서 주로 볼 수 있는 스타일이다.
언어에 따라 전부 카멜, 전부 파스칼로 표기할 것을 권장하기도 합니다.
'''
...
Python에서는 https://www.python.org/dev/peps/pep-0008/ 에 "Style Guide for Python Code"를 기술하고 있습니다.
위의 내용을 간단하게 설명하면 다음과 같습니다.
...
- 식별자의 길이는 제한이 없습니다.
- 파이썬은 대소문자를 구별하는 언어입니다. 이는, Variable과 variable은 동일하지 않습니다.
